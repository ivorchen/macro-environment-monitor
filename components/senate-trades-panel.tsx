...
function SourceLink({ transaction }: { transaction: NormalizedSenateTransaction }) {
  if (!transaction.filingUrl) return <span className="text-[#8b9691]">No link</span>;
  return (
    <a href={transaction.filingUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold text-[#175f47] hover:underline">
      Filing <ExternalLink className="size-3" />
    </a>
  );
}
  ...
  return (
    <div className="mx-auto max-w-[1500px] px-4 py-8 sm:px-7 lg:px-12 lg:py-10">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="mb-2 text-[10px] font-extrabold tracking-[0.2em] text-[#6f7d78]">DELAYED PUBLIC DISCLOSURES · AUDITABLE EVIDENCE</p>
          <h2 className="font-display text-4xl tracking-[-0.04em] sm:text-5xl">Senate trades</h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-[#66746e]">
            Compare disclosed direct-equity purchases across parties without treating delayed filings, categorical amounts, or family ownership as proof of intent.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex rounded-full border border-[#d4d9d3] bg-[#fbfaf6] p-1" aria-label="Disclosure window">
          ...
          <Button variant="outline" className="rounded-full" onClick={() => {
            ...
            <RefreshCw className={cn("size-4", status === "loading" && "animate-spin")} /> Refresh
          </Button>
        </div>
      </div>
      ...
      {status === "loading" && !payload && (
        <div className="grid min-h-64 place-items-center rounded-2xl border border-[#d9ddd7] bg-[#fbfaf6]">
          <LoaderCircle className="size-6 animate-spin text-[#718079]" aria-label="Loading Senate disclosures" />
        </div>
      )}
      ...
      {status === "error" && !payload && (
        <Card className="border-[#e3beb7] bg-[#f6e7e3] shadow-none">
          <CardContent className="grid min-h-48 place-items-center p-6 text-center text-sm text-[#813d35]">
            The Senate disclosure API route could not be loaded.
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-[#e3beb7] bg-[#f6e7e3] shadow-none">
          <CardContent className="grid min-h-52 place-items-center p-6 text-center">
            <div>
              ...
              <p className="font-display text-2xl text-[#813d35]">Senate disclosures are unavailable</p>
              <p className="mx-auto mt-2 max-w-xl text-xs leading-5 text-[#955d54]">{payload.errorMessage}</p>
              <p className="mt-3 text-[10px] text-[#955d54]">No rankings are generated when the source or quota ledger is unavailable.</p>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {payload && payload.status !== "unavailable" && (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {[
              ["Eligible purchases", String(payload.overview.eligiblePurchases)],
              ["Senator households", String(payload.overview.distinctSenatorHouseholds)],
              ["Bipartisan tickers", String(payload.overview.bipartisanTickers)],
              ["Median disclosure lag", payload.overview.medianDisclosureLagDays === null ? "—" : `${payload.overview.medianDisclosureLagDays} days`],
              ["Last refresh", formatRefresh(payload.lastSuccessfulRefresh)],
            ].map(([label, value]) => (
              <Card key={label} className="border-[#d9ddd7] bg-[#fbfaf6] shadow-none">
                <CardContent className="p-4">
                  ...
                  <p className={cn("mt-2 font-display text-2xl", label === "Last refresh" && "font-sans text-sm font-semibold leading-5")}>{value}</p>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,.55fr)]">
            <Card className="border-[#d9ddd7] bg-[#fbfaf6] shadow-none">
              <CardHeader>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="mb-2 text-[9px] font-extrabold tracking-[.18em] text-[#6f7d78]">DEMOCRATIC + REPUBLICAN HOUSEHOLDS</p>
                    <CardTitle className="font-display text-3xl">Bipartisan buys</CardTitle>
                  </div>
                  <Badge variant="outline" className="border-[#d4d9d3]">Transaction date · from {formatDate(payload.windowStart)}</Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-[#e4e6e2]">
                      {payload.bipartisan.map((item) => (
                        <tr key={item.ticker}>
                          ...
                          <td className="px-3">{formatDate(item.latestTradeDate)}</td>
                  ...
                  {!payload.bipartisan.length && (
                    <p className="py-10 text-center text-xs text-[#74817b]">No ticker meets the bipartisan purchase rule in this window.</p>
                  )}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-[#c8d6a7] bg-[#dfeabf] shadow-none">
              <CardHeader>
                <CardTitle className="font-display text-3xl">Popular by party</CardTitle>
                <p className="text-xs leading-5 text-[#58705f]">Ranked first by distinct senators, then events, recency, and ticker.</p>
              </CardHeader>
                ...
                {(["Democratic", "Republican", "Independent/Other"] as const).map((partyName) => (
                  <div key={partyName}>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <span className="text-[9px] text-[#68796e]">Top 10</span>
                    </div>
                      ...
                      {payload.popularByParty[partyName].map((item, index) => (
                        <li key={`${partyName}-${item.ticker}`}>
                          <button className="grid w-full grid-cols-[22px_1fr_auto] items-center gap-2 rounded-xl bg-white/30 px-2.5 py-2 text-left hover:bg-white/45" onClick={() => setSelectedTicker(item.ticker)}>
                            ...
                            <span><strong className="font-mono text-xs">{item.ticker}</strong><span className="ml-2 text-[9px] text-[#718079]">{item.purchaseEvents} events</span></span>
                            <span className="text-[10px] font-semibold">{item.distinctBuyers} buyers</span>
                          </button>
                        </li>
                      ))}
                      {!payload.popularByParty[partyName].length && <li className="text-[10px] text-[#718079]">No eligible purchases.</li>}
                    </ol>
                  </div>
                ))}
              </CardContent>
          ...
          {selectedTicker && (
            <Card className="mt-4 border-[#9fb9ad] bg-[#edf2ec] shadow-none">
              <CardHeader className="flex-row items-start justify-between gap-3 space-y-0">
                <div>
                  <p className="mb-2 text-[9px] font-extrabold tracking-[.18em] text-[#62756b]">TICKER DRILL-DOWN</p>
                  <CardTitle className="font-display text-3xl">{selectedTicker}</CardTitle>
                  <p className="mt-1 text-xs text-[#64756d]">Buyer, party, ownership, filing timeline, and disclosure lag.</p>
                </div>
                <Button variant="ghost" size="sm" onClick={() => setSelectedTicker(null)}>Close</Button>
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {tickerTransactions.map((transaction) => (
                  <div key={transaction.sourceId} className="rounded-2xl border border-[#ced9d1] bg-white/45 p-4 text-xs">
                    ...
                    <p className="mt-2 text-[10px] text-[#718079]">Trade {formatDate(transaction.transactionDate)} → disclosed {formatDate(transaction.disclosureDate)} ({transaction.disclosureLagDays} days)</p>
          ...
          <Card className="mt-4 border-[#d9ddd7] bg-[#fbfaf6] shadow-none">
            <CardHeader>
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div><p className="mb-2 text-[9px] font-extrabold tracking-[.18em] text-[#6f7d78]">SOURCE RECORDS RETAINED FOR AUDIT</p><CardTitle className="font-display text-3xl">Recent disclosures</CardTitle></div>
                <Badge variant="outline">{filteredTransactions.length} shown</Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
                <label className="relative lg:col-span-2">
                  ...
                  <Input className="pl-9" value={tickerQuery} onChange={(event) => setTickerQuery(event.target.value)} placeholder="Ticker" />
                </label>
                {[
                  { label: "Party", value: party, setValue: setParty, options: parties },
                  { label: "Owner", value: owner, setValue: setOwner, options: owners },
                  { label: "Transaction", value: transactionType, setValue: setTransactionType, options: transactionTypes },
                ].map((filter) => (
                  <label key={filter.label}>
                    ...
                    <select className="h-10 w-full rounded-md border border-[#d5dad5] bg-white/55 px-3 text-xs" value={filter.value} onChange={(event) => filter.setValue(event.target.value)}>
                      <option value="all">All {filter.label.toLowerCase()} values</option>
                  ...
                  <tbody className="divide-y divide-[#e4e6e2]">
                    {filteredTransactions.slice(0, 100).map((transaction) => (
                      <tr key={transaction.sourceId}>
                        <td className="py-3 pr-3"><strong>{formatDate(transaction.disclosureDate)}</strong><span className="mt-0.5 block text-[9px] text-[#7d8983]">Trade {formatDate(transaction.transactionDate)}</span></td>
              ...
              <p className="mt-3 text-[9px] leading-4 text-[#7c8983]">Sector filtering is withheld because the Senate endpoint does not supply a verified sector field; adding it requires a separately cached security-master enrichment.</p>
            </CardContent>
          </Card>
          ...
          <Card className="mt-4 border-[#dfcfaa] bg-[#f5ecd8] shadow-none">
            <CardContent className="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-start">
              <div>
                <p className="font-semibold text-[#6e5124]">Data quality and interpretation guardrails</p>
                <ul className="mt-2 list-disc space-y-1 pl-4 text-[10px] leading-4 text-[#805f2a]">
                  ...
                  <li>Disclosures can arrive up to 45 days after a qualifying transaction; amount categories remain ranges and ownership is always shown.</li>
                  <li>This descriptive module never changes the nine-pillar macro score automatically.</li>
                </ul>
              </div>
              <div className="grid grid-cols-2 gap-x-5 gap-y-1 text-[9px] text-[#805f2a]">
                <span>Raw records</span><strong>{payload.quality.rawRecords}</strong>
                <span>Normalized</span><strong>{payload.quality.normalizedRecords}</strong>
                <span>Duplicates</span><strong>{payload.quality.exactDuplicates}</strong>
                <span>Superseded</span><strong>{payload.quality.supersededVersions}</strong>
                <span>Unmapped members</span><strong>{payload.quality.unmappedMembers}</strong>
                <span>Unknown tickers</span><strong>{payload.quality.unknownTickers}</strong>
              </div>
            </CardContent>
          </Card>

          <div className="mt-3 flex flex-wrap gap-4 text-[9px] text-[#75827c]">
            <a href={payload.source.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:underline">FMP endpoint docs <ExternalLink className="size-3" /></a>
            <a href={payload.source.officialRosterUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:underline">Official Senate roster <ExternalLink className="size-3" /></a>
            <a href={payload.source.officialFilingDatabaseUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:underline">Official disclosure database <ExternalLink className="size-3" /></a>
